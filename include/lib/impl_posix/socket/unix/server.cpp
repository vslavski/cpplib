/* File: /lib/impl_posix/socket/unix/server.cpp
 *
...
// IMPLEMENTATION lib::socket::impl::unix::server

server::server( const cstring & filename ) {
	if ( not bind( filename ) )
		CPP_UNUSED( close() );
}
...
bool server::bind( const cstring & filename ) {
	...
	::std::strncpy( address.sun_path, filename.data(), sizeof(address.sun_path) - 1 );
...
bool server::close()/* override*/ {
	...
	auto result = check_error(
		::getsockname( sock, (struct sockaddr*)&address, &address_size ) );
	const char * path = result.success() ? address.sun_path : nullptr;

	if ( not Super::close() )
		return false;

	if ( path != nullptr and error() != ::std::make_error_condition( ::std::errc::address_in_use ) ) {
		result = check_error( ::unlink( path ) );
		if ( result.failed() )
			return false;
	}
